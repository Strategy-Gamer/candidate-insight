...
const data = JSON.parse(fs.readFileSync('kamala_harris.json', 'utf-8'));
    ...
    try {
        for (const position of data.positions) {
            ...
            if (issueRes.rows.length === 0) {
                throw new Error(`Issue ${position.issue} not found`);
            }
            ...
            const posRes = await client.query(
                ...
                [1, issueId, position.stance, position.desc]
            );
...
insertData().catch(err => console.error("Fatal error:", err.message));
